...
def main():
    df1 = pd.DataFrame(extractDigits(os.listdir("server/output")), columns=["website"])
    tracking = 0
    functional = 0
    for j in df1.index:
        try:
            ...
            res = pd.read_json(
                "server/output/" + df1["website"][j] + "/responses.json", lines=True
            )
            for i in df.index:
    ...
    print(
        "total tracking requests:", tracking, "total functional requests:", functional
    )
