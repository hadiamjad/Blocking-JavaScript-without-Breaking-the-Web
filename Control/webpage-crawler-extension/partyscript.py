...
def main():
    df1 = pd.DataFrame(
        extractDigits(
            os.listdir(
                "/home/student/TrackerSift/ASE-22/Control/webpage-crawler-extension/server/output"
            )
        ),
        columns=["website"],
    )
    ...
    for j in df1.index:
        try:
            ...
            df = pd.read_json(
                "server/output/" + df1["website"][j] + "/label_request.json"
            )
            for i in df.index:
                # try:
                if df["call_stack"][i]["type"] == "script":
                    if (
                        getInitiatorURL(df["call_stack"][i]["stack"])
                        not in script.keys()
                    ):
                        script[getInitiatorURL(df["call_stack"][i]["stack"])] = [
                            0,
                            0,
                            [],
                        ]
                    if (
                        df["top_level_url"][i]
                        not in script[getInitiatorURL(df["call_stack"][i]["stack"])][2]
                    ):
                        script[getInitiatorURL(df["call_stack"][i]["stack"])][2].append(
                            df["top_level_url"][i]
                        )
                    if (
                        df["easylistflag"][i] == 1
